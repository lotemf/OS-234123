...
//destroy was called and doesn't need to finish tasks
		if (tp->destroyFlag && !tp->finishAllFlag) {
			printf("[DEBUG-startThreadRoutine]\tinside startRoutine. destroy flag is on, finishAll flag is off\n");
			return NULL;
		}
//destroy was called and need to finish all tasks
		if (tp->destroyFlag && tp->finishAllFlag) {
			printf("[DEBUG-startThreadRoutine]\tinside startRoutine. destroy flag is on, finishAll flag is on\n");
			if (sem_trywait(sem) != 0) {
				//no tasks to finish
				//TODO consider again unlocking here
//				pthread_mutex_unlock(&(tp->tasksMutex));
				printf("[DEBUG-startThreadRoutine]\tinside startRoutine. trywait != 0\n");
				return NULL;
			} else{
				...
				if (!osIsQueueEmpty(tp->tasksQueue)) {
					...
					printf("[DEBUG-startThreadRoutine]\tinside startRoutine. trywait == 0 and tasks queue is empty\n");
					pthread_mutex_unlock(&(tp->tasksMutex));
					sem_post(&(tp->semaphore)); //CHEN - in oder to maintain correct semaphore value
					printf("[DEBUG-startThreadRoutine]\tinside startRoutine. trywait == 0 and tasks queue is empty after unlocking\n");
					return NULL;
				}
			}
		}
...
//		pthread_mutex_unlock(&(tp->tasksMutex));
		printf("[DEBUG-startThreadRoutine]\tBefore sem_wait(sem)\n");
		sem_wait(sem);
		printf("[DEBUG-startThreadRoutine]\tBefore destroyFlag check\n");
		if (!tp->destroyFlag) {
...
//step 7: creating threads
	...
	printf("[DEBUG-tpCreate]\t\tCreate is finished!\n");
...
//if destroy was called, we do not insert more tasks to the queue
	...
	if (!node){
		printf("[DEBUG-tpInsertTask]\tmemory allocation in FuncStruct node");
		return -1;
	}
...
//		pthread_mutex_unlock(&(tp->tasksMutex));
	...
	printf("[DEBUG-tpDestroy]\t\t\tin tpDestroy function before updating flags\n");
	tp->destroyFlag = true;
	tp->finishAllFlag = (shouldWaitForTasks != 0);

	int i;
	printf("[DEBUG-tpDestroy]\t\t\tin tpDestroy function before increasing semaphore counter\n");
...
//wait for all threads to finish their tasks/all tasks if needed
	...
	for(i = 0; i < tp->numOfThreads; ++i) {
		printf("[DEBUG-tpDestroy]\t\t\tinside pthread_join loop index is: %d - BEFORE\n", i);
		pthread_join((tp->threadsArray[i]), NULL);
		printf("[DEBUG-tpDestroy]\t\t\tinside pthread_join loop index is: %d - AFTER \n", i);
